...
Simplex defines fifteen landmarks. Five describe structure. Ten describe functions.
...
Of the fifteen landmarks, five are always required for a valid specification. One additional landmark is conditionally required.
...
**Design Note:** This is intentional and represents a research hypothesis. Simplex is designed for autonomous agent workflows where agents operate over extended periods, hours or days. The hypothesis is that agents can infer task dependencies and decomposition from context, potentially discovering structures the specification author did not anticipate. Prescribed composition would constrain this emergent behavior.
...
FUNCTION: parse_spec(text) → Spec
  ...
  RULES:
    ...
    - optional: DATA, CONSTRAINT, BASELINE, EVAL, READS, WRITES, TRIGGERS, NOT_ALLOWED, HANDOFF, UNCERTAIN
